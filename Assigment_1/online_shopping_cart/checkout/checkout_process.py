from Assigment_1.online_shopping_cart.checkout.shopping_cart import ShoppingCart
from Assigment_1.online_shopping_cart.product.product_data import get_products
...
from Assigment_1.online_shopping_cart.user.user_logout import logout
from Assigment_1.online_shopping_cart.user.user import User
    ...
    while True:
        ...
        if choice.startswith('d'):
            ...
            if check_cart(user=user, cart=global_cart) is False:
                ...
                users = UserDataManager.load_users() #Loads all users from users.json
                for u in users:
                    if u['username'] == user.name:
                        u['wallet'] = user.wallet # Updates wallet information
                        break
                UserDataManager.save_users(users) # Saves all the users with changes from users.json
